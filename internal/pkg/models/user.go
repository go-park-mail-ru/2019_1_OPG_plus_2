...
type SignInAnswerMessage struct {
    AnswerMessage
    Data SignInAnswer `json:"data" example:"login: test@mail.ru"`
}
...
type GetUserAnswerMessage struct {
    AnswerMessage
    Data GetUserAnswer `json:"data" example:"id:1,username:test,email:test@mail.ru,avatar:/static/1.jpg,score:1337,games:10,win:7,lose:3"`
}
...
func SendGetUserAnswer(w http.ResponseWriter, status int, message string, data GetUserAnswer) {
...
type ScoreboardAnswerMessage struct {
    AnswerMessage
    Data []ScoreboardUserData `json:"data" example:"[{id:1,username:test,score:1337},{id:2,username:user,score:228}]"`
}
